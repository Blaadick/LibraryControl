...
void UsersTableView::draw(WINDOW* window) {
    mvwprintw(window, 1, 2, "Id Name                     Phone              PassportId");

    for(auto i = 0; i < users.size(); ++i) {
        if(i == getSelectedRow()) {
            wattron(window, COLOR_PAIR(2));
        }
        mvwprintw(window, i + 3, 2, "%s", std::format("{:>2} {:<24} {:<18} {}", users[i].id, users[i].name, users[i].phoneNumber, users[i].passportId).c_str());
